...
public class Gl3Mesh implements ColladaMesh {
    ...
    public ColladaProfileEvaluator createProfileEvaluator(){
    	return new GLES30ProfileEvaluator();
    }
    ...
    public void createModel(){
    	...
    	GLES30.glBindBuffer( GLES30.GL_ARRAY_BUFFER, vboVertexHandle );
		...
		if( useNormals ){
	    	...
	    	GLES30.glBindBuffer( GLES30.GL_ARRAY_BUFFER, vboNormalsHandle );
		...
		if( useTexCoords ){
			...
			GLES30.glBindBuffer( GLES30.GL_ARRAY_BUFFER, vboTexCoordsHandle );
			GLES30.glBufferData( GLES30.GL_ARRAY_BUFFER, mTexCoordsBuffer.capacity()*4, mTexCoordsBuffer, GLES30.GL_STATIC_DRAW  );
			GLES30.glBindBuffer( GLES30.GL_ARRAY_BUFFER, 0 );
		}

		vboIndexHandle = glGenBuffer();
    			
		GLES30.glBindBuffer( GLES30.GL_ELEMENT_ARRAY_BUFFER, vboIndexHandle );
		GLES30.glBufferData( GLES30.GL_ELEMENT_ARRAY_BUFFER, mIndexBuffer.capacity()*4, mIndexBuffer, GLES30.GL_STATIC_DRAW );
...
//			"   gl_Normal = aNormal;                       \n" +
		...
		if( texName.equalsIgnoreCase("texDefault")){
			...
			fragmentShaderSrc =
			    ...
			    "void main()                                         \n" +
			    "{                                                   \n" +
			    "  fragmentColor = texture("+texName+", vTexCoord);  \n" +
			    "}                                                   \n";
		}
    ...
    public void drawPoints(Matrix4 proj, Matrix4 view){
    	drawPrimitive(GLES30.GL_POINTS, proj, view);
    }

    public void drawTriangles(Matrix4 proj, Matrix4 view){
    	drawPrimitive(GLES30.GL_TRIANGLES, proj, view);
    }
    ...
    public void drawPrimitive(int drawType, Matrix4 proj, Matrix4 view){
		...
		GLES30.glUniformMatrix4fv( modelViewLocation, 1, false, worldProjViewBuffer.getBuffer() );
		...
		if( useTexCoords ){
			...
			GLES30.glUniform1f( texLocation, 0 );
		}

		if( colorLocation != -1 ) {
			GLES30.glUniform4f(colorLocation, shader_red, shader_blue, shader_green, shader_alpha);
		}
		...
		GLES30.glBindBuffer( GLES30.GL_ELEMENT_ARRAY_BUFFER, vboIndexHandle );
		GLES30.glDrawElements( drawType, mIndexBuffer.capacity(), GLES30.GL_UNSIGNED_INT, 0 );
	...
	private class GLES30ProfileEvaluator implements ColladaProfileEvaluator{
	...
	public static int glGenFramebuffer()
	{
		...
		IntBuffer intbuf = tmp.asIntBuffer();
		intbuf.position(0);
		GLES30.glGenFramebuffers( 1, intbuf );
		return intbuf.get(0);
	}
	...
	public static int glGenRenderbuffer()
	{
		...
		IntBuffer intbuf = tmp.asIntBuffer();
		intbuf.position(0);
		GLES30.glGenRenderbuffers( 1, intbuf );
		return intbuf.get(0);
	}
	...
	public static int loadShader( int type, String source )
	{
		...
		GLES30.glGetShaderiv( shader, GLES30.GL_COMPILE_STATUS, intbuf );
		int compiled = intbuf.get(0);
		if( compiled == 0 )
		{					
			GLES30.glGetShaderiv( shader, GLES30.GL_INFO_LOG_LENGTH, intbuf );
			int infoLogLength = intbuf.get(0);
			if( infoLogLength > 1 )
			{
				String infoLog = GLES30.glGetShaderInfoLog( shader );
				Log.d( "GLES30", "shader info: " + infoLog );
			}
			throw new RuntimeException( "creating the shader didn't work" );
		}
	...
	public static int createProgram( int vertexShader, int fragmentShader )
	{
		...
		GLES30.glGetProgramiv( program, GLES30.GL_LINK_STATUS, intbuf );
		int linked = intbuf.get(0);
		if( linked == 0 )
		{
			GLES30.glGetProgramiv( program, GLES30.GL_INFO_LOG_LENGTH, intbuf );
			int infoLogLength = intbuf.get(0);
			if( infoLogLength > 1 )
			{
				Log.d( "GLES30", "couldn't link program: " + GLES30.glGetProgramInfoLog( program ) );
			}
